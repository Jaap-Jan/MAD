...
class WorkerOCR(WorkerBase):
    ...
    def _post_move_location_routine(self, timestamp):
        ...
        if not self._takeScreenshot():
            ...
            log.debug("Worker: couldn't take screenshot before radscreen check, lock released")
            return
        ...
        if self._applicationArgs.weather:
            ...
            weather = checkWeather(os.path.join(self._applicationArgs.temp_path, 'screenshot%s.png' % str(self._id)))
            if weather[0]:
                log.debug('Submit Weather')
                cell_id = S2Helper.lat_lng_to_cell_id(self.current_location.lat, self.current_location.lng)
                self._db_wrapper.update_insert_weather(cell_id, weather[1], timestamp)
            else:
    ...
    def _speed_weather_check_thread(self):
        while not self._stop_worker_event.is_set():
            ...
            try:
                ...
                log.error("Timeout during init of worker %s with %s" % (str(self._id), str(e)))
